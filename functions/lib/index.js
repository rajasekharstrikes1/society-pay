...
app.post("/api/verify-payment", authenticateUser, async (req, res) => {
    const { razorpay_order_id, razorpay_payment_id, razorpay_signature, subscriptionId, communityId, duration } = req.body;
    ...
    const generatedSignature = crypto
        ...
        .digest("hex");
    const isValid = crypto.timingSafeEqual(Buffer.from(generatedSignature), Buffer.from(razorpay_signature));
    if (!isValid) {
...
// 🔚 Export API Handler
export const api = onRequest({ timeoutSeconds: 60, memory: "256MiB", maxInstances: 10 }, app);
